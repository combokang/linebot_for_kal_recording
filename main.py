...
def message_text(event):
    ...
    DATABASE_URL = os.popen(
        "heroku config:get DATABASE_URL -a linebotforkal")
    ...
    # 記錄性別
        ...
        def_add_profile.add_gender(
            line_bot_api, conn, event, user_id, text, status)
    # 記錄身高
    elif status == "記錄性別":
        def_add_profile.add_high(
            line_bot_api, conn, event, user_id, text, status)
    # 紀錄體重
    elif status == "記錄身高":
        def_add_profile.add_weight(
            line_bot_api, conn, event, user_id, text, status)
    # 紀錄年齡
    elif status == "記錄體重":
        def_add_profile.add_age(
            line_bot_api, conn, event, user_id, text, status)
    # 紀錄活動量
    elif status == "記錄年齡":
        def_add_profile.add_activity(
            line_bot_api, conn, event, user_id, text, status)
    # 開始計算TDEE
    elif status == "記錄活動量":
        count_tdee.count_tdee(line_bot_api, conn,
                              event, user_id, text, status)
