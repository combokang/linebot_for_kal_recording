...
def prfile_record(line_bot_api, conn, event, user_id, text, status):
    ...
    if search_result is None:
        # 更新使用者搜尋狀態為新增 user_id、將user_id加入activities表
        SQL_order = f"""
        ...
        # 回傳訊息
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text='請輸入性別（男/女），或輸入"中止"')
        )
...
def add_gender(line_bot_api, conn, event, user_id, text, status):
    ...
    if text == "中止":
        SQL_order = f"""
        update userinfo set status = '中止' where userid = '{user_id}';
        """
        cursor.execute(SQL_order)
        conn.commit()
        print("SQL更新userinfo狀態:中止 成功")
        cursor.close()
        # 回傳訊息
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="已中止設定"))
    elif text in ["男", "女"]:
        gender = text
        print(f"輸入性別：{gender}")
        SQL_order = f"""
        ...
        # 回傳訊息
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text='請輸入身高（只需數字），或輸入"中止"')
        )
    else:  # 錯誤處理
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text='請輸入男/女，或輸入"中止"')
        )
...
def add_high(line_bot_api, conn, event, user_id, text, status):
    ...
    if text == "中止":
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token, TextSendMessage(text='請輸入數字，或輸入"中止"')
            )
...
def add_activity(line_bot_api, conn, event, user_id, text, status):
    ...
    if text == "中止":
        ...
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text='請輸入低/中/高，或輸入"中止"')
        )
